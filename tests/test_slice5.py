"""Slice 5 tests: SSE Urgency Alerts."""
...
def test_detect_alerts_color_degradation():
    ...
    results = [
        {"place_id": 1, "color": "yellow", "closing_urgency_minutes": 55, "slack_minutes": 10},
    ]
    last_colors = {1: "green"}
...
def test_detect_alerts_to_red_is_critical():
    ...
    results = [
        {"place_id": 1, "color": "red", "closing_urgency_minutes": 25, "slack_minutes": 2},
    ]
...
def test_detect_alerts_to_gray_is_critical():
    ...
    results = [
        {"place_id": 1, "color": "gray", "closing_urgency_minutes": None, "slack_minutes": -10},
    ]
    last_colors = {1: "yellow"}
    place_names = {1: "Museum A"}
    place_priorities = {1: "want"}
...
def test_detect_alerts_no_change_no_alert():
    ...
    results = [
        {"place_id": 1, "color": "green", "closing_urgency_minutes": 120, "slack_minutes": 60},
    ]
    last_colors = {1: "green"}
    place_names = {1: "Museum A"}
    place_priorities = {1: "want"}
...
def test_detect_alerts_must_visit_closing_soon():
    ...
    results = [
        {"place_id": 1, "color": "red", "closing_urgency_minutes": 25, "slack_minutes": 5},
    ]
...
def test_detect_alerts_must_visit_closing_within_hour():
    ...
    results = [
        {"place_id": 1, "color": "yellow", "closing_urgency_minutes": 45, "slack_minutes": 15},
    ]
...
async def test_stream_initial_feasibility(mock_compute, async_client):
    ...
    trip_payload = {
        "city": "Budapest",
        "start_lat": 47.4979, "start_lon": 19.0402,
        "end_lat": 47.5, "end_lon": 19.05,
        "start_time": "09:00", "end_time": "18:00",
        "date": "2026-04-15", "transport_mode": "foot",
    }
    ...
    mock_response = FeasibilityResponse(
        ...
        places=[
            FeasibilityResult(
                place_id=1, color="green",
                slack_minutes=60.0, closing_urgency_minutes=120.0,
                reason="plenty of time",
            )
        ],
    )
    mock_compute.return_value = (mock_response, {1: "Museum A"}, {1: "want"})
    ...
    async def limited_sleep(duration):
        ...
        call_count += 1
        if call_count >= 2:
            # Raise to break the SSE loop after 2 iterations
